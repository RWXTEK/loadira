-- Loadira initial database schema

-- Carriers table: one row per trucking company
...
-- Index for slug-based lookups (public profile pages)
...
-- Quote requests from brokers/shippers on public profile pages
...
-- Uploaded documents (W-9, COI, carrier agreements, etc.)
...
-- Carriers: owners can read/update their own, public can read by slug
...
-- Quote requests: anyone can insert, carrier owner can read
...
-- Documents: only carrier owner can CRUD
...
-- Storage bucket for carrier logos and documents
